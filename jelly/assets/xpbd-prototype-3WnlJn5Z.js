...
`;function ot(e,t,n){return Je(e,t,n)}var st=class e{device;vertexCount;bodyCount;parametersBuffer;flagsBuffer;readbackBuffer;clearPipeline;classifyPipeline;clearGroup;classifyGroup;constructor(e,t,n,r,i,a,o,s,c,l){this.device=e,this.vertexCount=t,this.bodyCount=n,this.parametersBuffer=r,this.flagsBuffer=i,this.readbackBuffer=a,this.clearPipeline=o,this.classifyPipeline=s,this.clearGroup=c,this.classifyGroup=l}static async create(t,n,r,i,a){let[o,s]=await Promise.all([Y(t,`body-release WGSL`,at,`clearFlags`),Y(t,`body-release WGSL`,at,`classifyVertices`)]),c=ot(t,new Uint8Array(16),rt.UNIFORM|rt.COPY_DST),l=ot(t,new Uint32Array(a),rt.STORAGE|rt.COPY_SRC),u=t.createBuffer({size:Math.max(4,a*4),usage:rt.MAP_READ|rt.COPY_DST}),d=[{binding:0,resource:{buffer:n}},{binding:1,resource:{buffer:r}},{binding:2,resource:{buffer:l}},{binding:3,resource:{buffer:c}}],f=X(t,{cacheKey:o,layout:o.getBindGroupLayout(0),entries:[{binding:2,resource:{buffer:l}},{binding:3,resource:{buffer:c}}]}),p=X(t,{cacheKey:s,layout:s.getBindGroupLayout(0),entries:d});return new e(t,i,a,c,l,u,o,s,f,p)}async readFlags(e){let t=new ArrayBuffer(16);new Float32Array(t)[0]=e,new Uint32Array(t)[1]=this.vertexCount,new Uint32Array(t)[2]=this.bodyCount,this.device.queue.writeBuffer(this.parametersBuffer,0,t);let n=this.device.createCommandEncoder(),r=n.beginComputePass();r.setPipeline(this.clearPipeline),r.setBindGroup(0,this.clearGroup),r.dispatchWorkgroups(Math.ceil(this.bodyCount/64)),r.end(),r=n.beginComputePass(),r.setPipeline(this.classifyPipeline),r.setBindGroup(0,this.classifyGroup),r.dispatchWorkgroups(Math.ceil(this.vertexCount/64)),r.end(),n.copyBufferToBuffer(this.flagsBuffer,0,this.readbackBuffer,0,this.bodyCount*4),this.device.queue.submit([n.finish()]),await this.readbackBuffer.mapAsync(it);let i=new Uint32Array(this.readbackBuffer.getMappedRange()).slice();return this.readbackBuffer.unmap(),i}destroy(){J(this.parametersBuffer),J(this.flagsBuffer),this.readbackBuffer.destroy()}},Z={COPY_SRC:4,COPY_DST:8,MAP_READ:1,STORAGE:128,UNIFORM:64},ct=`
...
`;function vt(e,t,n){return Je(e,t,n)}function yt(e,t,n){let r=[e[0]-t[0],e[1]-t[1],e[2]-t[2]],i=Math.hypot(...r);if(i<=1e-8)return[0,0,0];let a=Math.min(.9,n/i);return[r[0]*a,r[1]*a,r[2]*a]}function bt(e,t){let n=Math.hypot(...e),r=n>t?t/n:1;return[e[0]*r,e[1]*r,e[2]*r]}var xt=class e{device;vertexCount;maximumFrameTranslation;maximumReleaseSpeed;recordsBuffer;parametersBuffer;beginFramePipeline;projectPipeline;beginFrameGroup;projectGroup;state=`inactive`;bodyIndex=0;target=[0,0,0];projectedTarget=[0,0,0];grabOrigin=[0,0,0];releaseVelocity=[0,0,0];releaseAfterInitialize=!1;constructor(e,t,n,r,i,a,o,s,c,l){this.device=e,this.vertexCount=t,this.maximumFrameTranslation=n,this.maximumReleaseSpeed=r,this.recordsBuffer=i,this.parametersBuffer=a,this.beginFramePipeline=o,this.projectPipeline=s,this.beginFrameGroup=c,this.projectGroup=l}static async create(t,n,r,i,a,o,s,c){let l=vt(t,new Float32Array(n*4),gt.STORAGE|gt.COPY_DST),u=vt(t,new Uint8Array(64),gt.UNIFORM|gt.COPY_DST),[d,f]=await Promise.all([Y(t,`grab WGSL`,_t,`beginFrame`),Y(t,`grab WGSL`,_t,`projectAnchors`)]),p=[{binding:0,resource:{buffer:a}},{binding:1,resource:{buffer:o}},{binding:2,resource:{buffer:s}},{binding:3,resource:{buffer:l}},{binding:4,resource:{buffer:c}},{binding:5,resource:{buffer:u}}],m=X(t,{cacheKey:d,layout:d.getBindGroupLayout(0),entries:p}),h=X(t,{cacheKey:f,layout:f.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:a}},{binding:3,resource:{buffer:l}},{binding:5,resource:{buffer:u}}]});return new e(t,n,r,i,l,u,d,f,m,h)}get active(){return this.state===`active`}begin(e,t,n){if(e.length!==this.vertexCount*4)throw Error(`GPU grab records do not match the packed vertex count`);this.device.queue.writeBuffer(this.recordsBuffer,0,e),this.bodyIndex=t,this.target=[...n],this.projectedTarget=[...n],this.grabOrigin=[...n],this.releaseVelocity=[0,0,0],this.releaseAfterInitialize=!1,this.state=`initialize`}updateTarget(e){this.state!==`active`&&this.state!==`initialize`||(this.target=[...e])}end(e){this.state!==`active`&&this.state!==`initialize`||(this.releaseVelocity=bt(e,this.maximumReleaseSpeed),this.state===`initialize`?this.releaseAfterInitialize=!0:this.state=`release`)}uploadParameters(e,t){let n=new ArrayBuffer(64),r=new Float32Array(n),i=new Uint32Array(n);r.set(t,0),r.set(this.target,4),r.set(e===3?this.grabOrigin:this.releaseVelocity,8),i[12]=e,i[13]=this.vertexCount,i[14]=this.bodyIndex,this.device.queue.writeBuffer(this.parametersBuffer,0,n)}encodeFrameStart(e){if(this.state===`inactive`)return;let t=this.state===`initialize`?3:this.state===`active`?1:2,n=t===3?this.grabOrigin:this.projectedTarget,r=t===1||t===3?yt(this.target,n,this.maximumFrameTranslation):[0,0,0];this.uploadParameters(t,r),(t===1||t===3)&&(this.projectedTarget=[n[0]+r[0],n[1]+r[1],n[2]+r[2]]);let i=e.beginComputePass();i.setPipeline(this.beginFramePipeline),i.setBindGroup(0,this.beginFrameGroup),i.dispatchWorkgroups(Math.ceil(this.vertexCount/64)),i.end(),t===2?this.state=`inactive`:t===3&&(this.state=`active`,this.releaseAfterInitialize&&(this.state=`release`))}encodeProjection(e){if(this.state!==`active`)return;let t=e.beginComputePass();t.setPipeline(this.projectPipeline),t.setBindGroup(0,this.projectGroup),t.dispatchWorkgroups(Math.ceil(this.vertexCount/64)),t.end()}destroy(){J(this.recordsBuffer),J(this.parametersBuffer)}},St={COPY_SRC:4,COPY_DST:8,MAP_READ:1,STORAGE:128,UNIFORM:64},Ct=1,wt=`
struct Params {
  origin: vec4<f32>,
  direction: vec4<f32>,
  triangleCount: u32,
...
`;function Tt(e,t,n){return Je(e,t,n)}var Et=class e{device;triangleCount;parametersBuffer;resultBuffer;readbackBuffer;clearPipeline;distancePipeline;trianglePipeline;clearGroup;distanceGroup;triangleGroup;constructor(e,t,n,r,i,a,o,s,c,l,u){this.device=e,this.triangleCount=t,this.parametersBuffer=n,this.resultBuffer=r,this.readbackBuffer=i,this.clearPipeline=a,this.distancePipeline=o,this.trianglePipeline=s,this.clearGroup=c,this.distanceGroup=l,this.triangleGroup=u}static async create(t,n,r,i){let[a,o,s]=await Promise.all([Y(t,`ray-pick WGSL`,wt,`clearResult`),Y(t,`ray-pick WGSL`,wt,`findDistance`),Y(t,`ray-pick WGSL`,wt,`findTriangle`)]),c=Tt(t,new Uint8Array(48),St.UNIFORM|St.COPY_DST),l=Tt(t,new Uint32Array(2),St.STORAGE|St.COPY_SRC),u=t.createBuffer({size:8,usage:St.MAP_READ|St.COPY_DST}),d=[{binding:0,resource:{buffer:n}},{binding:1,resource:{buffer:r}},{binding:2,resource:{buffer:l}},{binding:3,resource:{buffer:c}}],f=X(t,{cacheKey:a,layout:a.getBindGroupLayout(0),entries:[{binding:2,resource:{buffer:l}}]}),p=X(t,{cacheKey:o,layout:o.getBindGroupLayout(0),entries:d}),m=X(t,{cacheKey:s,layout:s.getBindGroupLayout(0),entries:d});return new e(t,i,c,l,u,a,o,s,f,p,m)}async pick(e,t){let n=new ArrayBuffer(48),r=new Float32Array(n),i=new Uint32Array(n);r.set(e,0),r.set(t,4),i[8]=this.triangleCount,this.device.queue.writeBuffer(this.parametersBuffer,0,n);let a=this.device.createCommandEncoder(),o=a.beginComputePass();o.setPipeline(this.clearPipeline),o.setBindGroup(0,this.clearGroup),o.dispatchWorkgroups(1),o.end(),o=a.beginComputePass(),o.setPipeline(this.distancePipeline),o.setBindGroup(0,this.distanceGroup),o.dispatchWorkgroups(Math.ceil(this.triangleCount/64)),o.end(),o=a.beginComputePass(),o.setPipeline(this.trianglePipeline),o.setBindGroup(0,this.triangleGroup),o.dispatchWorkgroups(Math.ceil(this.triangleCount/64)),o.end(),a.copyBufferToBuffer(this.resultBuffer,0,this.readbackBuffer,0,8),this.device.queue.submit([a.finish()]),await this.readbackBuffer.mapAsync(Ct);let s=new Uint32Array(this.readbackBuffer.getMappedRange()).slice();return this.readbackBuffer.unmap(),s[1]===4294967295?null:{triangle:s[1],distance:new Float32Array(s.buffer,0,1)[0]}}destroy(){J(this.parametersBuffer),J(this.resultBuffer),this.readbackBuffer.destroy()}},Dt=`
struct Globals {
  dt: f32,
  vertexCount: u32,
  gravity: f32,
...
fn projectStretchEdge(firstIndex: u32, secondIndex: u32, restLength: f32) {
  ...
  let maximumLength = restLength * ${Se};
...
fn projectPlate(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  var position = positions[vertex];
  if (position.w <= 0.0) { return; }
  if (length(position.xz) > ${Ee}) { return; }
  let minimumY = plateHeight(position.x, position.z) + ${De};
  if (position.y >= minimumY) { return; }
  let previousPosition = previous[vertex];
  let previousMinimumY = plateHeight(previousPosition.x, previousPosition.z) + ${De};
