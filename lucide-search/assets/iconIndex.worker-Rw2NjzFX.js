(function(){var e=`<?xml version="1.0" encoding="utf-8"?>
...
<svg xmlns="http://www.w3.org/2000/svg" version="1.1">
  <defs>
    ...
    <symbol id="angry" viewBox="0 0 24 24">
    ...
    <symbol id="annoyed" viewBox="0 0 24 24">
      <circle cx="12" cy="12" r="10" />
      <path d="M8 15h8" />
      <path d="M8 9h2" />
      <path d="M14 9h2" />
    ...
    <symbol id="smile-plus" viewBox="0 0 24 24">
...
`;let t=Math.SQRT2;function n(e,n,r){let i=new Float32Array(n*r);for(let t=0;t<e.length;t+=1)i[t]=e[t]===1?0:1e6;for(let e=0;e<r;e+=1)for(let r=0;r<n;r+=1){let a=e*n+r,o=i[a];r>0&&(o=Math.min(o,i[a-1]+1)),e>0&&(o=Math.min(o,i[a-n]+1)),r>0&&e>0&&(o=Math.min(o,i[a-n-1]+t)),r<n-1&&e>0&&(o=Math.min(o,i[a-n+1]+t)),i[a]=o}for(let e=r-1;e>=0;--e)for(let a=n-1;a>=0;--a){let o=e*n+a,s=i[o];a<n-1&&(s=Math.min(s,i[o+1]+1)),e<r-1&&(s=Math.min(s,i[o+n]+1)),a<n-1&&e<r-1&&(s=Math.min(s,i[o+n+1]+t)),a>0&&e<r-1&&(s=Math.min(s,i[o+n-1]+t)),i[o]=s}return i}function r(e){let t=[];for(let n=0;n<e.length;n+=1)e[n]===1&&t.push(n);return t}function i(e){let t=e.size,n=t/8,r=t*t*2+t*2+64,i=new Float32Array(r),o=0;for(let t=0;t<e.binary.length;t+=1)i[o]=e.binary[t],o+=1;for(let t=0;t<e.distanceMap.length;t+=1)i[o]=Math.min(e.distanceMap[t]/16,1),o+=1;for(let n=0;n<t;n+=1){let r=0;for(let i=0;i<t;i+=1)r+=e.binary[n*t+i];i[o]=r/t,o+=1}for(let n=0;n<t;n+=1){let r=0;for(let i=0;i<t;i+=1)r+=e.binary[i*t+n];i[o]=r/t,o+=1}for(let r=0;r<8;r+=1)for(let a=0;a<8;a+=1){let s=Math.floor(a*n),c=Math.floor((a+1)*n),l=Math.floor(r*n),u=Math.floor((r+1)*n),d=0,f=0;for(let n=l;n<u;n+=1)for(let r=s;r<c;r+=1)d+=e.binary[n*t+r],f+=1;i[o]=f===0?0:d/f,o+=1}return a(i),i}function a(e){let t=0;for(let n=0;n<e.length;n+=1)t+=e[n]*e[n];if(t=Math.sqrt(t),t!==0)for(let n=0;n<e.length;n+=1)e[n]/=t}function o(e,t,n,r=64){let i=d(e.data,t,n);if(!i)return c(r);let a=u(t,n),o=a.getContext(`2d`,{willReadFrequently:!0});return o?(o.putImageData(e,0,0),s(l(a,i,r).getImageData(0,0,r,r),r)):c(r)}function s(e,t){let a=new Uint8Array(t*t),o=e.data;for(let e=0;e<a.length;e+=1){let t=e*4,n=o[t+3],r=(o[t]+o[t+1]+o[t+2])/3;a[e]=+(n>8&&r<180)}let s={size:t,binary:a,distanceMap:n(a,t,t),strokePixels:r(a)};return{...s,embedding:i(s)}}function c(e){let t=new Uint8Array(e*e),r={size:e,binary:t,distanceMap:n(t,e,e),strokePixels:[]};return{...r,embedding:i(r)}}function l(e,t,n){let r=u(n,n).getContext(`2d`,{willReadFrequently:!0});if(!r)throw Error(`Canvas 2D context is not available`);r.fillStyle=`white`,r.fillRect(0,0,n,n),r.imageSmoothingEnabled=!0;let i=t.maxX-t.minX+1,a=t.maxY-t.minY+1,o=(n-12)/Math.max(i,a),s=i*o,c=a*o,l=(n-s)/2,d=(n-c)/2;return r.drawImage(e,t.minX,t.minY,i,a,l,d,s,c),r}function u(e,t){if(typeof document>`u`)return new OffscreenCanvas(e,t);let n=document.createElement(`canvas`);return n.width=e,n.height=t,n}function d(e,t,n){let r=t,i=n,a=-1,o=-1;for(let s=0;s<n;s+=1)for(let n=0;n<t;n+=1){let c=(s*t+n)*4,l=e[c+3],u=(e[c]+e[c+1]+e[c+2])/3;l>8&&u<180&&(r=Math.min(r,n),i=Math.min(i,s),a=Math.max(a,n),o=Math.max(o,s))}return a<r||o<i?null:{minX:r,minY:i,maxX:a,maxY:o}}let f=_(e).sort((e,t)=>e.name.localeCompare(t.name));self.onmessage=async e=>{let t=e.data.size,n=[];for(let e=0;e<f.length;e+=16){let r=f.slice(e,e+16),i=await Promise.all(r.map(async e=>{let n=p(e,t);return{name:e.name,uri:e.uri,...o(n,t,t,t)}}));n.push(...i)}self.postMessage(n)};function p(e,t){let n=new OffscreenCanvas(t,t).getContext(`2d`,{willReadFrequently:!0});if(!n)throw Error(`Canvas 2D context is not available`);return n.fillStyle=`white`,n.fillRect(0,0,t,t),m(n,e,t),n.getImageData(0,0,t,t)}function m(e,t,n){let[r,i,a,o]=t.viewBox.split(/\s+/).map(Number),s=n/Math.max(a,o);e.save(),e.scale(s,s),e.translate(-r,-i),e.strokeStyle=`black`,e.fillStyle=`transparent`,e.lineWidth=2,e.lineCap=`round`,e.lineJoin=`round`;for(let[,n,r]of t.content.matchAll(/<(path|circle|rect|line|polyline|polygon|ellipse)\b([^>]*)>/g)){let t=v(r);n===`path`?e.stroke(new Path2D(t.d)):n===`circle`?(e.beginPath(),e.arc(y(t,`cx`),y(t,`cy`),y(t,`r`),0,Math.PI*2),e.stroke()):n===`ellipse`?(e.beginPath(),e.ellipse(y(t,`cx`),y(t,`cy`),y(t,`rx`),y(t,`ry`),0,0,Math.PI*2),e.stroke()):n===`rect`?h(e,t):n===`line`?(e.beginPath(),e.moveTo(y(t,`x1`),y(t,`y1`)),e.lineTo(y(t,`x2`),y(t,`y2`)),e.stroke()):(n===`polyline`||n===`polygon`)&&g(e,t.points,n===`polygon`)}e.restore()}function h(e,t){let n=y(t,`x`),r=y(t,`y`),i=y(t,`width`),a=y(t,`height`),o=Number(t.rx??t.ry??0);e.beginPath(),o>0?e.roundRect(n,r,i,a,o):e.rect(n,r,i,a),e.stroke()}function g(e,t,n){let r=(t??``).trim().split(/\s+/).map(e=>e.split(`,`).map(Number)).filter(([e,t])=>Number.isFinite(e)&&Number.isFinite(t));if(r.length!==0){e.beginPath(),e.moveTo(r[0][0],r[0][1]);for(let[t,n]of r.slice(1))e.lineTo(t,n);n&&e.closePath(),e.stroke()}}function _(e){let t=e.matchAll(/<symbol\s+id="([^"]+)"\s+viewBox="([^"]+)"\s*>([\s\S]*?)<\/symbol>/g);return Array.from(t,([,e,t,n])=>{let r=`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="${t}" fill="none" stroke="black" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${n}</svg>`;return{content:n,name:e,svg:r,viewBox:t,uri:`data:image/svg+xml;charset=utf-8,${encodeURIComponent(r)}`}})}function v(e){return Object.fromEntries(Array.from(e.matchAll(/([\w:-]+)="([^"]*)"/g),([,e,t])=>[e,t]))}function y(e,t){return Number(e[t]??0)}})();